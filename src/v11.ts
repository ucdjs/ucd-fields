...
export interface BidiMirroring {
  bidi_mirroring_glyph: "none" | string; // The Bidi_Mirroring_Glyph property for each code point, indicating the mirrored character or 'none' if no suitable character exists.
  code_point: string; // The code point represented as a variable-length hexadecimal value.
...
export interface CjkRadicals {
  cjk_radical_number: string; // The CJK radical number, which matches the regular expression [1-9][0-9]{0,2} and can end with an apostrophe.
...
export interface EmojiSources {
  unicode_code_point_or_sequence: string; // The Unicode code point or sequence representing the character or symbol.
  docomo_shift_jis_code: string; // The Shift-JIS code for DoCoMo carrier symbols equivalent to the Unicode character.
  kddi_shift_jis_code: string; // The Shift-JIS code for KDDI carrier symbols equivalent to the Unicode character.
  softbank_shift_jis_code: string; // The Shift-JIS code for SoftBank carrier symbols equivalent to the Unicode character.
}
...
export interface EquivalentUnifiedIdeograph {
  ...
  mapped_code_point: string; // The equivalent CJK unified ideograph represented as a variable-length hexadecimal value with 4 to 6 digits.
...
export interface IndicPositionalCategory {
  unicode_code_point: string; // Unicode code point value or range of code point values
  indic_positional_category: string; // Indic positional category property value, indicating the placement of dependent vowels and similar characters in Indic scripts.
}
...
export interface LineBreak {
  ...
  line_break_property: "BK" | "CM" | "CR" | "GL" | "LF" | "NL" | "SP" | "WJ" | "ZW" | "ZWJ" | "AI" | "AL" | "B2" | "BA" | "BB" | "CB" | "CJ" | "CL" | "CP" | "EB" | "EM" | "EX" | "H2" | "H3" | "HL" | "HY" | "ID" | "IN" | "IS" | "JL" | "JT" | "JV" | "NS" | "NU" | "OP" | "PO" | "PR" | "QU" | "RI" | "SA" | "SG" | "SY" | "XX"; // Line_Break property, indicating how the character behaves in line breaking situations.
}
...
export interface SpecialCasing {
  ...
  condition_list: string[]; // A list of conditions that override the normal behavior for casing, represented as language IDs or casing contexts
...
export interface USourceData {
  ...
  status: "C" | "D" | "E" | "F" | "G" | "N" | "U" | "UNC-2013" | "UNC-2015" | "UK-2015" | "V" | "W" | "WS-2017" | "X" | "UTC-xxxxx" | "UCI-xxxxx"; // Text indicating the ideograph's status
