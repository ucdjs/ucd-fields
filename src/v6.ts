...
export interface ArabicShaping {
  code_point: string; // The code point of an Arabic, Syriac, or N'Ko character, in 4-digit hexadecimal form
...
export interface BidiMirroring {
  bidi_mirroring_glyph: string; // The code point of the character that is the mirror image of the original character's glyph.
  code_point: string; // The code point of the character that has the Bidi_Mirrored=True property.
}
...
export interface CjkRadicals {
  radical_number: string; // The radical number, which matches the regular expression [0-9]{1,3}'?
...
export interface IndicMatraCategory {
  code_point: string; // The code point of a character, in hexadecimal form
  indic_matra_category: string; // The category of the Indic matra, which defines its role in syllabic structure
}
...
export interface IndicSyllabicCategory {
  code_point: string; // The code point of a character, in hexadecimal form
  indic_syllabic_category: string; // The Indic syllabic category of the character, defining its role in syllable structure and segmentation rules.
}
...
export interface LineBreak {
  ...
  line_break_property: "BK" | "CR" | "LF" | "CM" | "SG" | "GL" | "CB" | "SP" | "ZW" | "NL" | "WJ" | "JL" | "JV" | "JT" | "H2" | "H3" | "XX" | "OP" | "CL" | "CP" | "QU" | "NS" | "EX" | "SY" | "IS" | "PR" | "PO" | "NU" | "AL" | "ID" | "IN" | "HY" | "BB" | "BA" | "SA" | "AI" | "B2"; // The line break property of the character, which can be one of several normative or informative values.
}
...
export interface NameAliases {
  code_point: string; // The code point of a Unicode character, represented in hexadecimal form.
  alias: string; // The alias assigned to the Unicode character.
}
...
export interface NamedSequencesProv {
  ...
  code_point_sequence: number[]; // A sequence of code points representing the named sequence.
}
...
export interface NormalizationCorrections {
  unicode_code_point: string; // The Unicode code point of the character, represented in hexadecimal form.
...
export interface PropertyAliases {
  ...
  preferred_aliases: string[]; // Preferred aliases for the property.
  additional_aliases: string[]; // Additional aliases that may be useful.
}
...
export interface PropertyValueAliases {
  ...
  additional_aliases: string[]; // Additional aliases for the property value, if any.
}
...
export interface ScriptExtensions {
  script_extensions: string[]; // A list of script extensions that characters belong to, represented as space-delimited short values.
}
...
export interface SpecialCasing {
  ...
  condition_list: string[]; // A list of conditions that override normal behavior, represented as language IDs or contexts
...
export interface DerivedDecompositionType {
  type_name: string; // The name of the derived decomposition type
}
...
export interface DerivedJoiningType {
  derived_joining_type: string; // The derived joining type of a character as defined in the Unicode Character Database.
