...
export interface CjkRadicals {
  cjk_radical_number: string; // The CJK radical number, which can match the regular expression [1-9][0-9]{0,2} and may end with an apostrophe.
...
export interface EmojiSources {
  unicode_code_point_or_sequence: string; // The Unicode code point or sequence for the character or symbol.
...
export interface PropertyAliases {
  ...
  other_aliases: string[]; // Other aliases that may be useful.
}
...
export interface USourceData {
  ...
  status: "C" | "D" | "E" | "F" | "G" | "N" | "U" | "UNC-2013" | "UNC-2015" | "UK-2015" | "V" | "W" | "X" | "(UTC-\d{5})|(UCI-\d{5})"; // Text indicating the ideograph's status
  ...
  virtual_kangxi_dictionary_position: number; // Virtual KangXi dictionary position
