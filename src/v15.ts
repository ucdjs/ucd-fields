...
export interface EquivalentUnifiedIdeograph {
  equivalent_unified_ideograph: string; // The property defining the equivalent unified ideograph for a given code point or range.
  code_point: string; // A code point or code point range represented as a variable-length hexadecimal value.
  mapped_code_point: string; // The equivalent CJK unified ideograph represented as a variable-length hexadecimal value.
...
export interface LineBreak {
  unicode_code_point: string; // Unicode code point value or range of code point values
  line_break_property: "BK" | "CM" | "CR" | "GL" | "LF" | "NL" | "SP" | "WJ" | "ZW" | "ZWJ" | "AI" | "AL" | "B2" | "BA" | "BB" | "CB" | "CJ" | "CL" | "CP" | "EB" | "EM" | "EX" | "H2" | "H3" | "HL" | "HY" | "ID" | "IN" | "IS" | "JL" | "JT" | "JV" | "NS" | "NU" | "OP" | "PO" | "PR" | "QU" | "RI" | "SA" | "SG" | "SY" | "XX"; // Line_Break property, indicating how the character should be treated in line breaking
}
...
export interface NamedSequencesProv {
  name_of_sequence: string; // The name of the Unicode named character sequence
  code_point_sequence: string[]; // The sequence of code points corresponding to the named character sequence
}
...
export interface ScriptExtensions {
  code_point: string; // The code point of a character, in hexadecimal form
  script_extensions: string[]; // A list of script property values associated with the code point
}
...
export interface SpecialCasing {
  ...
  condition_list: string[]; // A list of conditions that override normal behavior, defined by language IDs or casing contexts
...
export interface USourceData {
  ...
  status: "ExtA" | "ExtB" | "ExtC" | "Comp" | "ExtD" | "ExtE" | "ExtF" | "ExtG" | "ExtH" | "N" | "URO" | "UK-2015" | "V" | "W" | "WS-2017" | "WS-2021" | "X" | "UTC-xxxxx" | "UCI-xxxxx"; // Indicates the ideograph's status regarding encoding and submission
  unicode_code_point: string; // A Unicode code point representing the encoded character or related variant
  ...
  sources: string[]; // Sources from which the ideograph is derived or referenced
  general_comments: string; // General comments regarding the ideograph
  total_strokes: number; // The total number of strokes in the ideograph
  first_residual_stroke: number; // The index of the first residual stroke in the ideograph
}
