...
export interface BidiMirroring {
  ...
  bidi_mirroring_glyph: "none" | string; // The character that is the mirror image of the original character's glyph, or none if no suitable character exists
}
...
export interface CjkRadicals {
  cjk_radical_number: string; // The CJK radical number, which matches the regular expression [1-9][0-9]{0,2}'?
  cjk_radical_character: string; // The CJK radical character corresponding to the radical number
  cjk_unified_ideograph: string; // The CJK unified ideograph formed from the radical only
}
...
export interface CompositionExclusions {
  code_point: string; // The code point of a character, in hexadecimal form
  name: string; // A short schematic name for the character
  composition_exclusion: boolean; // Indicates whether the character is excluded from composition
}
...
export interface EmojiSources {
  unicode_code_point_or_sequence: string; // Unicode code point or sequence
  docomo_shift_jis_code: string; // DoCoMo Shift-JIS code for the equivalent symbol
  kddi_shift_jis_code: string; // KDDI Shift-JIS code for the equivalent symbol
  softbank_shift_jis_code: string; // SoftBank Shift-JIS code for the equivalent symbol
}
...
export interface EquivalentUnifiedIdeograph {
  equivalent_unified_ideograph: string; // The property defining the equivalent unified ideograph for a given code point.
  code_point: string; // A code point or code point range represented as a variable-length hexadecimal value with 4 to 6 digits.
  equivalent_code_point: string; // A code point represented as a variable-length hexadecimal value with 4 to 6 digits that is equivalent to the first code point.
  default_value: "none" | string; // The default value of the Equivalent_Unified_Ideograph property, which is 'none' unless specified otherwise.
}
...
export interface IndicPositionalCategory {
  unicode_code_point: string; // Unicode code point value or range of code point values
  indic_positional_category: string; // Indic Positional Category property value, indicating the positional category of the character in Indic scripts.
}
...
export interface LineBreak {
  ...
  line_break_property: "BK" | "CM" | "CR" | "GL" | "LF" | "NL" | "SP" | "WJ" | "ZW" | "ZWJ" | "AI" | "AL" | "B2" | "BA" | "BB" | "CB" | "CJ" | "CL" | "CP" | "EB" | "EM" | "EX" | "H2" | "H3" | "HL" | "HY" | "ID" | "IN" | "IS" | "JL" | "JT" | "JV" | "NS" | "NU" | "OP" | "PO" | "PR" | "QU" | "RI" | "SA" | "SG" | "SY" | "XX"; // Line_Break property, indicating how line breaking should be handled for the code point
}
...
export interface NameAliases {
  code_point: string; // The code point of a character, in hexadecimal form
  alias: string; // The alias for the Unicode character
  type: "correction" | "control" | "alternate" | "figment" | "abbreviation"; // The type of alias defined for the Unicode character
}
...
export interface NamedSequencesProv {
  name_of_sequence: string; // The name of the character sequence.
  code_point_sequence: string[]; // A sequence of code points representing the character sequence.
}
...
export interface PropertyAliases {
  ...
  additional_aliases: string[]; // Other aliases that may be useful for the property.
}
...
export interface ScriptExtensions {
  code_point: string; // The code point of a character, in hexadecimal form
  script_extensions: string[]; // A list of script property values associated with the code point, indicating which characters are commonly used with more than one script.
}
...
export interface SpecialCasing {
  code: string; // The Unicode code point of the character, expressed in hexadecimal form
...
export interface USourceData {
  ...
  status: "A" | "B" | "C" | "Comp" | "D" | "E" | "F" | "G" | "N" | "U" | "UK-2015" | "V" | "W" | "WS-2017" | "X" | "UTC-xxxxx" | "UCI-xxxxx"; // Indicates the ideograph's status regarding encoding
  unicode_code_point: string; // A Unicode code point representing the ideograph
  radical_stroke_count: number; // The count of radical strokes for the ideograph
  virtual_kangxi_position: number; // The virtual position in the KangXi dictionary
  ideographic_description_sequence: string; // The Ideographic Description Sequence (IDS) if available
  sources: string[]; // The source(s) from which the ideograph is derived
  general_comments: string; // General comments regarding the ideograph
}
...
export interface DerivedBidiClass {
  derived_bidi_class: string; // The derived bidirectional class of a character as defined in the Unicode Character Database.
  date: string; // The date when the file was generated.
  copyright: string; // Copyright information for the Unicode data.
  unicode_character_database: string; // Documentation reference for the Unicode Character Database.
}
...
export interface EmojiData {
  codepoint: string; // The code point of the emoji character, in hexadecimal form
  property: string; // The property of the emoji character
  comments: string; // Additional comments or information about the emoji character
}
...
export interface EmojiVariationSequences {
  code_point: string; // The code point of the emoji variation sequence, in hexadecimal form
  emoji: string; // The emoji character that the variation sequence modifies
  variation_selector: string; // The variation selector used to indicate the desired emoji variation
}
